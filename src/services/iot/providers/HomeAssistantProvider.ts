...
export class HomeAssistantProvider extends EventEmitter {
  ...
  token: string;
  messageId: number;
  pendingRequests: Map<number, Function>;
  ...
  async connect(): Promise<boolean> {
    ...
    this.connectionPromise = new Promise((resolve) => {
      ...
      try {
        ...
        this.ws.on("close", () => {
          console.log(
            "[HA_PROVIDER] WebSocket closed. Scheduling reconnect..."
          );
          this.ws = null;
          this.connectionPromise = null;
        ...
        this.once("authenticated", async (success: boolean) => {
          if (success) {
  ...
  async getDevices(): Promise<SmartDevice[]> {
    ...
    for (const entity of response.result) {
      ...
      if (device) {
        devices.push(device);
        this.devices.set(device.id, device); // Assuming id exists on SmartDevice
      }
    }
  ...
  mapEntityToDevice(entity: any): SmartDevice | null {
    ...
    return {
      ...
      type: type as any, // Cast to SmartDeviceType
      isOn:
        entity.state === "on" ||
  ...
  async controlDevice(deviceId: string, action: string, params: any = {}) {
