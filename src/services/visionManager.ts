...
export class VisionManager {
   ...
   */
  private getDefaultConfig(): VisionConfig {
   ...
   */
  private detectIntent(instruction: string): VisionIntent {
    ...
    if (
      planningPatterns.some((pattern) => lowerInstruction.includes(pattern))
    ) {
      return "planning";
    }
   ...
   */
  private validateEnvironment(geminiApiKey?: string) {
...
Format: {"action": "click/type/scroll", "coordinates": {"x": 0, "y": 0}, "text": "optional"}`;
   ...
   */
  updateConfig(newConfig: Partial<VisionConfig>) {
