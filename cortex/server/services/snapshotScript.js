 ...
 * Adapted from SawyerHood/dev-browser for Luca.
 */
...
(function() {
  ...
  function renderAriaTree(ariaSnapshot) {
    ...
    const createKey = (ariaNode, renderCursorPointer) => {
      ...
      if (ariaNode.name) key += " " + yamlEscapeValueIfNeeded(ariaNode.name);
      if (ariaNode.ref) key += " [ref=" + ariaNode.ref + "]";
      if (ariaNode.active) key += " [focused]";
      if (renderCursorPointer && ariaNode.box.cursor === "pointer" && ariaNode.role !== "link") key += " [clickable]";
      if (ariaNode.checked !== undefined) key += " [checked=" + ariaNode.checked + "]";
      if (ariaNode.disabled !== undefined) key += " [disabled=" + ariaNode.disabled + "]";
      if (ariaNode.expanded !== undefined) key += " [expanded=" + ariaNode.expanded + "]";
      if (ariaNode.level !== undefined) key += " [level=" + ariaNode.level + "]";
      if (ariaNode.pressed !== undefined) key += " [pressed=" + ariaNode.pressed + "]";
      if (ariaNode.selected !== undefined) key += " [selected=" + ariaNode.selected + "]";
      for (const [k, v] of Object.entries(ariaNode.props)) key += " /" + k + ": " + yamlEscapeValueIfNeeded(v);
      return key;
    };

    const visitNode = (ariaNode, indent) => {
      const key = createKey(ariaNode, true);
      const isContainer = ariaNode.children.length > 0;
      if (isContainer) {
        lines.push(indent + "- " + key + ":");
        for (const child of ariaNode.children) {
          if (typeof child === "string") visitText(child, indent + "  ");
          else visitNode(child, indent + "  ");
        }
      } else {
        lines.push(indent + "- " + key);
      }
    ...
    const rootChildren = ariaSnapshot.root.role === "fragment" ? ariaSnapshot.root.children : [ariaSnapshot.root];
    for (const child of rootChildren) {
       if (typeof child === "string") visitText(child, "");
  ...
  // Store the last snapshot to allow element lookup by ref
  window.__devBrowser_lastSnapshot = null;

  function getAISnapshot() {
    if (!document.body) return "Page loading...";
    const snapshot = generateAriaTree(document.body);
    window.__devBrowser_lastSnapshot = snapshot;
    return renderAriaTree(snapshot);
  }

  function selectSnapshotRef(ref) {
    const snapshot = window.__devBrowser_lastSnapshot;
